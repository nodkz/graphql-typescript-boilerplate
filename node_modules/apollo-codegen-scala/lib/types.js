...
function typeNameFromGraphQLType(context, type, bareTypeName, isOptional, isInputObject) {
    if (graphql_1.isNonNullType(type)) {
        return typeNameFromGraphQLType(context, type.ofType, bareTypeName, isOptional || false, isInputObject);
    }
    ...
    if (graphql_1.isListType(type)) {
        if (isInputObject) {
            typeName =
                "Seq[" +
                    typeNameFromGraphQLType(context, type.ofType, bareTypeName, undefined, isInputObject) +
                    "]";
        }
        else {
            typeName =
