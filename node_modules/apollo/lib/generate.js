...
function toPath(uri) {
    return vscode_uri_1.default.parse(uri).path;
}
function generate(document, schema, outputPath, only, target, tagName, nextToSources, options) {
    ...
    if (target === "swift") {
        ...
        if (outputIndividualFiles) {
            ...
            localfs_1.fs.writeFileSync(outputPath, generator.output);
            writtenFiles += 1;
        }
        ...
        if (nextToSources) {
            generatedFiles.forEach(({ sourcePath, fileName, content }) => {
                const dir = path.join(path.dirname(path.posix.relative(rootPath, toPath(sourcePath))), outputPath);
        ...
        if (nextToSources ||
            ...
            const globalSourcePath = options.globalTypesFile || path.join(outputPath, "globalTypes.ts");
            ...
            generatedFiles.forEach(({ sourcePath, fileName, content }) => {
                let dir = outputPath;
                if (nextToSources) {
                    dir = path.join(path.dirname(path.posix.relative(rootPath, toPath(sourcePath))), dir);
                    if (!localfs_1.fs.existsSync(dir)) {
                        localfs_1.fs.mkdirSync(dir);
                    }
                }
...
function writeGeneratedFiles(generatedFiles, outputDirectory) {
    for (const [fileName, generatedFile] of Object.entries(generatedFiles)) {
        localfs_1.fs.writeFileSync(path.join(outputDirectory, fileName), generatedFile.output);
    }
}
